...
| `GET` | `/api/v1/experience/fno/portfolio-hedge` | Hedge wizard data — BS put + call pricing per holding at given coverage. Params: `coverage` 0–100 (default 50), `total_value_eur` (optional float). `duration` param removed (F29 Phase 0) — tenor hardcoded to 1y (12 months). Returns `PortfolioHedgeResponse`: holdings with `cost_pct` (put premium), `call_sell_cost_pct` (call income), `ann_vol_pct`, `risk_score`, `strike_pct`, `strike_label`, `protected_pct`, `duration` (always `"1y"`); plus `aggregate` and `coverage`. Used by 2-tab hedge wizard (Discover/Hedge) in FnO. | JWT | `experience/portfolio_hedge.py` |
