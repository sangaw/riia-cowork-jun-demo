...
| `GET` | `/api/v1/experience/fno/portfolio-hedge` | Hedge wizard data — BS put + call pricing per holding at given coverage/duration. Params: `coverage` 0–100 (default 50), `duration` 1m\|3m\|1y (default 1y). Returns `PortfolioHedgeResponse`: holdings with `cost_pct` (put premium), `call_sell_cost_pct` (call income), `ann_vol_pct`, `risk_score`, `strike_pct`, `strike_label`, `protected_pct`, `duration`; plus `aggregate` and `coverage`. Used by 4-tab hedge wizard (Discover/Selection/Allocation/Hedge) in FnO. | JWT | `experience/portfolio_hedge.py` |
