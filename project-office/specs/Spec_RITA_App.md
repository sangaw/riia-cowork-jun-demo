...
| `GET` | `/api/v1/experience/fno/portfolio-analytics?mode=real\|mock` | Unified FnO dashboard analytics payload — positions, greeks, net_greeks, net_delta, scenario_levels, payoff (21-pt grid), stress (5 events), hedge_quality (HQS per instrument), market OHLCV, closed_positions, realized_pnl, margin. mode=mock: 200 no auth (demo data); mode=real: JWT required → 401 without token, 404 if no portfolio. Greeks derived from saved hedge plan; if no plan: delta=1.0, theta/vega/gamma=0. Added F30 Phase 1. | JWT (mode=real only) | `experience/portfolio_analytics.py` |
