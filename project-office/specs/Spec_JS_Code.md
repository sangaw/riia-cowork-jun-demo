...
| `app-init.js` | Single-fetch init via `/api/v1/experience/fno/portfolio-analytics?mode=`. F30 Phase 2: `initApp(mode='mock')` replaces separate fetch chain; `fetchPositions()` is a backward-compat shim that calls `initApp(state.analyticsMode)`. **F30 Phase 3:** adds `_normScenarioLevels(raw)` — normalises `{target,sl}` shape to `{bull:{target,sl},bear:{target:sl,sl:target}}`; applied to `data.scenario_levels` before state assignment. `_renderAll()` now calls `renderOverviewFromState()` (my-portfolio.js) and `renderPortfolioHedgeRadar()` (hedge.js) instead of `renderHedgeRadar()`. | `fetchPositions()`, `initApp(mode?)`, `checkStatus()` |
