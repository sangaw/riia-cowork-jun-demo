...
| `portfolio-hedge.js` | 2-tab hedge wizard (Feature 28 Phase 3; F29 Phase 0 duration cleanup) — Discover (holdings summary + σ scenarios) → Hedge (coverage dial + payoff simulator). Duration picker removed; tenor hardcoded to 1y. State: `_state.{tab, coverage, holdings, instruments, apiHedge, selections, reached}`. API: `GET /api/v1/experience/fno/portfolio-hedge?coverage=N` (JWT). | `loadPortfolioHedge()`, `phGoNext()`, `phGoBack()`, `phGoToTab(tab)`, `phPickStrategy(id, strategy)`, `phSetCoverage(val)` |
