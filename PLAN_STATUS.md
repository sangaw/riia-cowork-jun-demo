...
**Last updated:** 2026-05-26 — Bug fixes: MCP timestamps EU format + investor onboarding flow auto-select + horizon stops. All pushed to prod. Phase 0 + Phase 1 live in production. EC2 local build used to bypass GitHub Actions runner queue freeze (PATTERN-011). Mobile UA → HTTP 302 → `/mobile` verified. Ops skill + SPEC_Prod_Deploy.md updated with EC2 local build procedure and frozen queue recovery.
